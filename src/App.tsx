...
import { FileText, ChevronRight, CornerDownRight, TreePine, ArrowUp } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'nosotros', 'proyectos', 'materiales', 'planificador', 'servicios', 'contacto'];
  ...
  const handleExportEstimate = (summary: string) => {
    ...
    // Scroll directly to contact forms smoothly
    setTimeout(() => {
  ...
  return (
    <div id="aura-app-root" className="min-h-screen bg-stone-950 font-sans text-stone-100 selection:bg-emerald-400 selection:text-stone-950 antialiased overflow-x-hidden">
      ...
      <main className="relative z-10">
        ...
        {/* 2. Nosotros & Servicios combined for clean bento layouts */}
        <AboutAndServices />

        {/* 3. Featured Luxury Projects Section */}
        <ProjectsSection onSelectProject={setSelectedProject} />

        {/* 4. Tactile Showroom Materials Library Section */}
        <MaterialLibrary onInquireMaterial={handleQuickInquire} />

        {/* 5. Investment Budget Planner Segment */}
        <InvestmentPlanner onExportEstimate={handleExportEstimate} />

        {/* 6. Legal / Custom Contact Information Entry Forms */}
        <ContactForm
      ...
      {/* Modal Dialog for Project Details (AnimatePresence) */}
      <AnimatePresence>
        {selectedProject && (
          <ProjectModal
      ...
      {/* Modal Dialog for Dossier / Catalog Brochures (AnimatePresence) */}
      <AnimatePresence>
        {isCatalogOpen && (
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            className="fixed bottom-6 right-6 z-40 p-3 bg-stone-900 hover:bg-stone-800 border border-stone-800 hover:border-emerald-500/50 text-stone-300 hover:text-emerald-400 transition-all rounded-none outline-none group"
